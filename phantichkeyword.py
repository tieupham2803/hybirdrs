...
print(list(md))
print(md.shape)
md['vote_count'] = pd.to_numeric(md['vote_count'], errors='coerce') #fix loi ValueError: invalid literal for int() with base 10: '7.6'
...
# print(build_chart('Crime').head(15))
...
s.name = 'keyword'
s = s.value_counts() # so lan xuat hien cua keyword
print(s[:5])
s = s[s > 1]
...
# print(s)
# s.to_csv('listkeyword.csv', sep=',', encoding='utf-8')
# print(smd['keywords'].head(10))
# # quit()
	# smd['original_language'][i] = smd['original_language'][i]+smd['original_language'][i]+smd['original_language'][i]

i = 0
# while i < len(smd['original_language']):
while i < 10:
	smd['original_language'][i] = smd['original_language'][i]+smd['original_language'][i]+smd['original_language'][i]
	# print(smd['original_language'][i])
	i += 1
smd['original_language'] = smd['original_language'].astype('str').apply(lambda x: str.lower(x.replace(" ", "")))
smd['original_language'] = smd['original_language'].apply(lambda x: [x,x])
# quit()
print(type(smd['original_language'][1]))
# smd['soup'] = smd['keywords'] + smd['cast'] + smd['director'] + smd['genres']
smd['soup'] = smd['keywords'] + smd['cast'] + smd['director'] + smd['genres'] + smd['original_language']
print(smd['soup'][1])
...
titles = smd['title']
indices = pd.Series(smd.index, index=smd['title'])
print(get_recommendations('Ashes of Time').head(10))
quit()
...
svd.train(trainset)
# print(ratings[ratinggs['userId'] == 554])
...
vonglap = 2
while(vonglap<10):
	print(hybrid(554, 'Chungking Express'))
	# vonglap=vonglap+3
	vonglap=int(input("nhap vao s"))
	print(vonglap)